...
function populateResourcesTable() {
    ...
    tasks.forEach(task => {
        ...
        // Simulación de estado y progreso (puedes conectar con datos reales o Power BI)
        let estado = 'En progreso';
        let progreso = Math.floor(Math.random() * 101); // 0-100%
        let observaciones = '';
        // Ejemplo de lógica ambiental avanzada
        if (task.custom_class === 'gantt-bar-evacuation') {
            estado = progreso > 80 ? 'Completado' : 'En progreso';
            observaciones = progreso < 50 ? 'Riesgo operativo, revisar EPP.' : '';
        } else if (task.custom_class === 'gantt-bar-medical') {
            estado = progreso > 90 ? 'Validado' : 'Pendiente';
            observaciones = progreso < 60 ? 'Falta validación médica.' : '';
        } else {
            estado = progreso > 90 ? 'Documentado' : 'En progreso';
            observaciones = progreso < 40 ? 'Documentación incompleta.' : '';
        }

        // Color de estado
        let estadoColor = '';
        if (estado === 'Completado' || estado === 'Validado' || estado === 'Documentado') {
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-3 whitespace-nowrap text-sm"><span class="px-2 py-1 rounded ${estadoColor}">${estado}</span></td>
            <td class="px-4 py-3 whitespace-nowrap text-sm">
                <div class="w-32 bg-gray-200 rounded-full h-3">
                    <div class="bg-blue-600 h-3 rounded-full" style="width: ${progreso}%"></div>
                </div>
                <span class="text-xs ml-2">${progreso}%</span>
            </td>
            <td class="px-4 py-3 text-sm text-gray-700">${observaciones}</td>
        `;
    ...
    try {
        const canvas = await html2canvas(ganttElement, {
            ...
            onclone: (clonedDoc) => {
                ...
                // Show the advanced table for export
                const resourcesTable = clonedDoc.getElementById('resources-table');
                if (resourcesTable) resourcesTable.style.display = 'block';

                clonedDoc.body.classList.add('preparing-for-export');

                // Make sure the Gantt chart and table are fully visible
                ...
                /*
                // Add a visual title and date to the export
                let reportTitle = document.createElement('div');
                reportTitle.style.fontSize = '2rem';
                reportTitle.style.fontWeight = 'bold';
                reportTitle.style.marginBottom = '12px';
                reportTitle.style.textAlign = 'center';
                reportTitle.textContent = 'Reporte Visual Cronograma Evacuación Semisólidos CAMI';
                main.prepend(reportTitle);

                let reportDate = document.createElement('div');
                reportDate.style.fontSize = '1rem';
                reportDate.style.marginBottom = '18px';
                reportDate.style.textAlign = 'center';
                reportDate.textContent = 'Fecha de generación: ' + (new Date()).toLocaleDateString('es-EC');
                main.prepend(reportDate);

                // Add summary stats (total tareas, completadas, pendientes)
                let total = document.querySelectorAll('#resources-table-body tr').length;
                let completadas = Array.from(document.querySelectorAll('#resources-table-body tr')).filter(row => row.innerHTML.includes('Completado')).length;
                let pendientes = total - completadas;
                let resumen = document.createElement('div');
                resumen.style.fontSize = '1rem';
                resumen.style.marginBottom = '18px';
                resumen.style.textAlign = 'center';
                resumen.textContent = `Total tareas: ${total} | Completadas: ${completadas} | Pendientes: ${pendientes}`;
